...
begin
    ...
    -- Afficher_Deux_Positions (2);     --cette procédure n'est pas documentée dans la spécification du module donc elle ne peut pas être utilisée
    New_Line;
    ...
    -- Une_Date.jour := 15;              -- le type date est privé, on ne connait pas sa structure donc on ne peut pas lui affecter des valeurs
    -- Une_Date.Mois := Mois_Suivant;    -- le type date est privé, on ne connait pas sa structure donc on ne peut pas lui affecter des valeurs
    Initialiser (Une_Date, 15, Mois_Suivant, L_Annee(Une_Date));
    Afficher (Une_Date);
    New_Line;
